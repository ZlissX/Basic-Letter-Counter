while True:
    ...
    if choose_method==3:
        confirm_exit=input("are you sure you want to exit the application?(reply with yes or no)").lower()
        if confirm_exit=="yes":
           print(("Exiting 0"or"Exiting ="or "Exiting-" or "Exiting=" or "Exiting0")*90000)
           exit()
        if confirm_exit=='no':
            print("restarting")
        if confirm_exit!="yes"or"no":
